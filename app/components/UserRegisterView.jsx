...
class UserRegisterView extends React.PureComponent {
    ...
    dataValid() {
        return (this.validNickname() &&
            ...
            this.state.termsAccepted && this.state.policyAccepted
    }
    ...
    handleChangeOfField(field, value) {
        if (/([A-zÀ-ú0-9!()\\-`.+,/\"]+|\s|[\b])$/.test(value) || value === ""|| field === PASSWORD || field === CONFIRM_PASSWORD || field === EMAIL || field === CONFIRM_EMAIL) {
            switch (field) {
                case APODO:
                    this.setState((prevState) => ({
                        dataChange: true,
